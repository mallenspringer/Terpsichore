...
| **Signal Latency/Dead Zones:** Triggers and fast LFOs feel "dead" or miss frames in the Renderer. | **Fix:** Implemented high-frequency signal bypass (`latestSignals`) to feed the Renderer directly from the engine loop, bypassing the 60ms React Store commit interval. |
| **Graph View Snap-back:** Interacting with a module far from the top-left corner snaps the view back to (0,0). | **Fix:** Decoupled scroll reset from node state updates in `NodeGraph.tsx` and added `preventDefault()` to buttons/tabs to block browser "focus-driven" scrolling in transformed containers. |
| **Module Drag Clunkiness:** Dragging modules/edges to off-screen areas requires multiple "chunked" moves. | **Fix:** Implemented **Autoscroll** in `ModuleNode` and `NodeGraph` that triggers when the cursor hits a 40px boundary during an active drag. |
| **Silent Rotation Input:** TriggerPad failed to visibly rotate ShapeGenerator. | **Fix:** Standardized `ShapeGenerator` rendering to use `getEffectiveParam` and ensured the modulation path correctly applies the 360-degree rotation scaling. |
